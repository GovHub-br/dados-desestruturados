...
def candidate_contract_instruction() -> str:
    """Explica como interpretar contrato, paths, arrays e entradas esperadas."""
    return (
        "O proximo bloco contem a semantica do contrato, a estrutura permitida do schema "
        "de saida, os alvos que voce deve mapear e, quando presente, entradas_esperadas: a "
        "lista fechada das chaves de mapeamento_canonico, ja montada pelo pipeline a partir "
        "do contrato e da identidade do documento. Com entradas_esperadas no bloco, as chaves "
        "da sua resposta sao exatamente as chaves listadas: copie-as sem alterar filtros, "
        "valores de filtro, ordem dos segmentos ou grafia; nao crie chave que nao esteja na "
        "lista e nao omita uma entrada com obrigatorio=true. Cada entrada diz o requisito de "
        "origem, o campo que a chave preenche (papel_no_alvo valor para o dado bruto, contexto "
        "para o campo que o acompanha na mesma observacao, livre para um campo opcional fora "
        "dos requisitos) e o modo_array (item: a chave aponta para um campo de um item "
        "filtrado; colecao: a chave e o proprio array, preenchido por uma unica "
        "linhas_de_tabela). Quando uma entrada trouxer filtros_pendentes, o valor <evidencia> "
        "daquele filtro e o unico que voce decide: substitua-o pelo rotulo lido no artefato, "
        "exatamente como aparece, e crie uma instancia da chave para cada rotulo que o "
        "documento publica para aquele requisito; os demais filtros da mesma chave ficam como "
        "estao. Uma entrada com obrigatorio=false e mapeada quando houver evidencia e omitida "
        "sem erro quando nao houver. Se um requisito obrigatorio nao tiver evidencia, nao "
        "invente chave alternativa: declare a ausencia em campos_nao_mapeados com o path e os "
        "seletores da entrada. Sem entradas_esperadas no bloco, monte as chaves voce mesmo: "
        "uma entrada para cada conjunto de seletores de observacoes_obrigatorias, com todos os "
        "campos_contexto_obrigatorios na mesma observacao; um array e mapeado ou por item, com "
        "filtro [chave=valor] em cada segmento de array, ou como colecao inteira pelo path raiz "
        "do array sem filtro, nunca os dois modos; indices posicionais como [0] sao invalidos. "
        "Em qualquer caso, paths_permitidos ja exclui valores fixos do contrato, que a DAG 2 "
        "preenche deterministicamente. Nao invente campos, contratos ou valores."
    )
...
def candidate_artifacts_instruction() -> str:
    ...
    return (
        ...
        "Todo array listado em arrays_que_exigem_seletor, inclusive os aninhados dentro de outro "
        "array, exige o proprio filtro [chave=valor] no segmento correspondente do path; a "
        "chave de cada array e a declarada em estrutura_schema_saida.chaves_de_item. "
        "origem_das_chaves diz de onde vem o valor de cada filtro: identidade_documento "
        "(a entidade e o periodo do proprio documento; nao pesquise nem varie esse valor "
        "entre requisitos do mesmo documento), seletor_observacao (o valor e exatamente o "
        "seletor da observacao no contrato, por exemplo o nome do papel, nunca um rotulo "
        "literal do documento) ou evidencia (o rotulo lido no artefato). Quando o contrato "
        "declarar requisitos_mapeamento.papeis, o filtro usa o nome do papel, nao o rotulo "
        "do cabecalho, e a coluna e escolhida pela descricao do papel. Quando declarar "
        "evidencia_esperada, use o tipo_origem indicado para cada campo. "
        "Se uma evidencia obrigatoria nao estiver presente, "
...
def candidate_layout_repair_system_prompt() -> str:
    ...
    return (
        ...
        "mapeamento_canonico. Quando o payload trouxer entradas_esperadas, as chaves de "
        "mapeamento_canonico sao exatamente as dessa lista (so o marcador <evidencia> e "
        "substituido pelo rotulo lido); fora disso, as chaves devem apontar somente "
        ...
        "arrays aninhados, e nunca concatene condicoes com '&' dentro de um filtro. A chave de "
        "cada filtro e a declarada em estrutura_schema_saida.chaves_de_item; origem_das_chaves "
        "diz de onde vem o valor: identidade_documento (entidade e periodo do proprio documento), "
        "seletor_observacao (exatamente o seletor da observacao no contrato, como o nome do "
        "papel, nunca um rotulo literal do documento) ou evidencia (rotulo lido no artefato). "
        "Com requisitos_mapeamento.papeis declarado, o filtro usa o nome do papel e a coluna e "
        "escolhida pela descricao do papel; com evidencia_esperada, use o tipo_origem indicado "
        "para cada campo. "
