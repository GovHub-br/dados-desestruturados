...
UNIT_MAPPING_SCOPE_TEMPLATE = (
    ...
    "ou por campo interno. Os arrays em unidade_mapeamento.colecoes_por_evidencia "
    "tem a chave do item vinda do artefato. Use linhas_de_tabela neles somente "
    "quando o artefato publica uma linha por item e a chave do item (a de "
    "chaves_de_item, por exemplo periodo) esta numa coluna: a instrucao cobre todas "
    "essas linhas, le a chave e o valor por indice_coluna em campos, e deixa em "
    "valores_por_segmento ou valores_fixos apenas o contexto constante, nunca a "
    "chave. O path da colecao mantem os filtros dos arrays ancestrais e nao tem "
    "filtro no proprio array, por exemplo grupo.dados[chave=valor].itens; o "
    "filtro do array pai e obrigatorio. Quando o artefato publica o item numa "
    "linha cujo rotulo e o indicador, com os periodos nas colunas, nao e colecao: "
    "mapeie cada observacao com celula_de_tabela, seletor_linha.valor_aceito com "
    "o rotulo da linha e o filtro [chave=valor] do item. "
...
def unit_mapping_artifacts_instruction() -> str:
    ...
    return (
        ...
        "chave de cada array e a declarada em chaves_de_item do subesquema; se chaves_de_item "
        "nao vier, use um campo do item daquele array, nunca de um array ancestral. "
        "origem_das_chaves diz de onde vem o valor de cada filtro: identidade_documento "
        "(a entidade e o periodo do proprio documento), seletor_observacao (o valor e "
        "exatamente o seletor da observacao no contrato, por exemplo o nome do papel, nunca "
        "um rotulo literal do documento) ou evidencia (o rotulo lido no artefato). Quando o "
        "contrato declarar requisitos_mapeamento.papeis, o filtro usa o nome do papel e a "
        "coluna e escolhida pela descricao do papel. Quando declarar evidencia_esperada, use "
        "o tipo_origem indicado para cada campo. Cada "
